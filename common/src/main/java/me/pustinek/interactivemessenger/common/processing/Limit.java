...
public class Limit {
	 ...
	 */
	public Limit(int count, IMessage message) {
		this.left = count;
		this.depth = 0;
	...
	public void decrease() throws ReplacementLimitReachedException {
		...
		if(left <= 0) {
			if(!notified) {
				notified = true;
				Log.error("Reached replacement limit, probably has replacements loops, problematic message key: " + message.getKey() + ", first characters of the message: " + IMessage.getMessageStart(message, 200));
			}
